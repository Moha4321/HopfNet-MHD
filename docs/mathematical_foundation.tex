...
\tableofcontents
\newpage
\section{Normalization and Alfvén Units}
\label{sec:units}
To ensure computational well-posedness, all physical quantities in the pseudo-spectral engine are normalized to dimensionless \textbf{Alfvén units}. We define a characteristic length scale $L_0$ (such that the triply periodic simulation box is length $L = 2\pi$), a characteristic magnetic field $B_0$, and a characteristic mass density $\rho_0$. 

From these, the Alfvén velocity is $v_A = B_0 / \sqrt{\mu_0 \rho_0}$ and the Alfvén transit time is $t_A = L_0 / v_A$. We normalize variables as follows:
\begin{equation}
    \mathbf{x} \to \frac{\mathbf{x}}{L_0}, \quad t \to \frac{t}{t_A}, \quad \mathbf{B} \to \frac{\mathbf{B}}{B_0}, \quad \mathbf{v} \to \frac{\mathbf{v}}{v_A}, \quad \rho \to \frac{\rho}{\rho_0}
\end{equation}
Under this normalization, the vacuum permeability becomes $\mu_0 = 1$. The Hall parameter scales as $d_i / L_0$, and the hyper-resistivity $\eta_4$ and hyper-viscosity $\nu_4$ become inverse Lundquist and Reynolds numbers at the grid scale, strictly isolating the dimensionless physical regime.
...
\subsection{The $k=0$ Singularity and Mean-Field Dynamics}
Equation (18) reveals that the operator acting on $\hat{\mathbf{R}}$ is the \textbf{Orthogonal Projection Tensor}, $\mathcal{P}_{ij}(\mathbf{k}) = \delta_{ij} - \frac{k_i k_j}{|\mathbf{k}|^2}$. 

Operationally, this tensor presents a $0/0$ singularity at the zero-mode ($\mathbf{k} = \mathbf{0}$). Physically, the $k=0$ mode represents the volume-averaged mean magnetic field of the simulation box. Because a spatially uniform mean field has zero gradient, it trivially satisfies $\nabla \cdot \mathbf{A} = 0$. Therefore, at $\mathbf{k} = \mathbf{0}$, we explicitly define the projection tensor as the identity matrix ($\mathcal{P}(\mathbf{0}) \equiv \mathbb{I}$). In our specific Hopf-link setup, the background mean field is zero, meaning the $k=0$ mode is completely fixed and carries no dynamics.

Equation (18) mathematically proves that we do \textit{not} need to evaluate the scalar potential $\phi$. By transforming the raw physical forces into Fourier space and multiplying by $\mathcal{P}_{ij}$, we perfectly strip away longitudinal components, ensuring $\nabla \cdot \mathbf{A} = 0$ is preserved to absolute machine precision ($10^{-15}$).

\subsection{Hermitian Symmetry and the Orszag 2/3 Dealiasing Rule}
To compute the nonlinear cross-products (e.g., $\mathbf{v} \times \mathbf{B}$), the engine must transform the spectral fields back to real space, multiply them, and transform the result back to $k$-space. Because $\mathbf{A}(\mathbf{x})$ is a strictly real-valued physical field, its Fourier coefficients exhibit Hermitian symmetry: $\hat{\mathbf{A}}(-\mathbf{k}) = \hat{\mathbf{A}}^*(\mathbf{k})$. Our C++ architecture leverages a Real-to-Complex Fast Fourier Transform (rFFT), which omits the redundant negative frequencies, cutting the $128^3$ RAM footprint exactly in half—a crucial optimization for executing high-fidelity dynamics entirely within local Unified Memory.

However, multiplying two spatial grids in real space causes a convolution in Fourier space. A quadratic nonlinearity multiplying modes $k_1$ and $k_2$ generates energy at $k_1 + k_2$. If $k_1 + k_2 > k_{max}$ (the Nyquist limit of the grid), this energy unphysically "folds back" into the resolved low-frequency spectrum, triggering a catastrophic aliasing instability. 

To eliminate this, we strictly enforce the \textbf{Orszag 2/3 Rule}. Before computing any nonlinear product, we apply a sharp spectral mask, zeroing out all Fourier modes where $|\mathbf{k}| > \frac{2}{3}k_{max}$. While this reduces the effective resolution of our $128^3$ grid to $\approx 85^3$, it mathematically guarantees an alias-free cascade, isolating pure physics from numerical artifact.
...
\subsection{The Floating-Point Singularity in RK4 Coefficients}
To evaluate the nonlinear integral numerically using a 4th-order Runge-Kutta scheme (ETDRK4), we must calculate specific coefficient matrices for the sub-steps. These include the first-order Euler step $f_1$, and the higher-order corrections $f_2$ and $f_3$:
\begin{align}
    f_1(\mathcal{L}) &= \frac{e^{\mathcal{L}\Delta t} - I}{\mathcal{L}} \\
    f_2(\mathcal{L}) &= \frac{e^{\mathcal{L}\Delta t} - \mathcal{L}\Delta t - I}{\mathcal{L}^2 \Delta t^2} \\
    f_3(\mathcal{L}) &= \frac{e^{\mathcal{L}\Delta t} - \frac{1}{2}\mathcal{L}^2 \Delta t^2 - \mathcal{L}\Delta t - I}{\mathcal{L}^3 \Delta t^3}
\end{align}

\textbf{The Computational Vulnerability:} These functions present catastrophic floating-point vulnerabilities. As $k \to 0$, the linear operator $\mathcal{L} \to 0$. Equations (31-33) evaluate strictly to $0/0$. Even for small wavenumbers ($k \approx 1$), $\mathcal{L}$ is very close to zero, causing severe \textit{catastrophic cancellation} in double-precision arithmetic. A single `NaN` generated here will instantly infect the entire grid upon the next inverse FFT.
...
Note that $\mathbf{A}^*$ inherits the gauge freedom of $\mathbf{A}$: since $\nabla \times \mathbf{v}$ does not constrain the longitudinal part of $\mathbf{A}^*$, its absolute value carries the same gauge ambiguity as $H$ itself. However, because our Coulomb-gauge projection $\mathcal{P}(\mathbf{k})$ is applied consistently at $t=0$ and the gauge choice is held fixed thereafter (no re-projection of $\phi$ is performed mid-step beyond what Section \ref{sec:gauge} already enforces), $H_{gen}$ is computed on a single, self-consistent gauge slice throughout the run. We therefore track \textit{changes} in $H_{gen}$ relative to its $t=0$ value as the physically meaningful diagnostic, rather than its absolute magnitude.
...
\section{Algorithmic Validation Strategy}
\label{sec:validation}
Before the engine is deployed to generate datasets for the Neural ODE, it must pass a suite of absolute physics verifications:
\begin{enumerate}
    \item \textbf{Divergence-Free Machine Precision:} Following initialization and every RK4 step, the condition $\max(|\nabla \cdot \mathbf{A}|) < 10^{-13}$ must hold.
    \item \textbf{Temporal Convergence:} By halving the timestep $\Delta t$, the global error of the nonlinear ETDRK4 integration must scale as $\mathcal{O}(\Delta t^4)$.
    \item \textbf{Inviscid Invariant Conservation:} By temporarily setting $\eta_4 = 0$ and $d_i=0$ (Ideal MHD limit), Magnetic Helicity $H$ and Energy $W$ must be conserved globally to $<0.1\%$ error over a full dynamic transit time.
    \item \textbf{Energy Spectrum Integrity:} The 1D isotropic magnetic energy spectrum $E(k)$ must be computed. It must exhibit a physical turbulent cascade (e.g., Kolmogorov $k^{-5/3}$) in the inertial range, with no unphysical energy pile-up near $k_{max}$, definitively proving the success of the Orszag 2/3 dealiasing rule.
\end{enumerate}
